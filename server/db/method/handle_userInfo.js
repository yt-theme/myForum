...
class HandleUserInfo {
    ...
    // 添加 users_info 内容
    insert () { let user_id=this.user_id,  name=this.name,  logo=this.logo,  toy=this.toy,  checkin=this.checkin
        return new Promise ((resolve, reject) => {
            new handle.Handle_usersInfo_table({ "user_id": user_id, "name": name, "logo": logo, "toy": toy, "checkin": checkin }).insert().then((v) => { resolve(v) }).catch((v) => { reject(v) }) })
    }
